...
- `workspace_status` 的 `shared-runtime/*`、`local-session.json`、`LocalSessionClaim`
...
- `hcodex` launcher script 仍串接：
  - `ensure-hcodex-runtime`
  - `resolve_hcodex_launch.py`
  - path-bearing ws URL 的 `hcodex-ws-bridge`
  - `run-hcodex-session`
- `resolve_hcodex_launch.py` 仍會讀 bot-local thread binding，並在存在 `tui_proxy_base_ws_url` 時組出 `/thread/<thread_key>` sideband URL
...
- [`resolve_hcodex_launch.py`](/Volumes/Data/Github/threadBridge/tools/resolve_hcodex_launch.py#L61)
- [`resolve_hcodex_launch.py`](/Volumes/Data/Github/threadBridge/tools/resolve_hcodex_launch.py#L107)
- [`hcodex_ws_bridge.rs`](/Volumes/Data/Github/threadBridge/rust/src/hcodex_ws_bridge.rs#L66)
...
1. 先完成 `app-server ws mirror observer`
   ...
   - 不再把 `TUI proxy` 當成主要 read-side intake
2. 再收斂 live session / adoption 判斷
   - 讓 Telegram 與其他 surface 不再依賴 `local-session.json` / local claim 去決定 canonical continuity
...
3. 再收斂 `hcodex` launch contract
   ...
   - `resolve_hcodex_launch.py`、`/thread/<thread_key>` sideband、`hcodex-ws-bridge` 應視為待縮減的 transition shim
