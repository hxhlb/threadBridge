...
- 目前行為與設計說明見 [codex-plan-mirror.md](../app-server-observer/codex-plan-mirror.md)
...
2. 依 [codex-plan-mirror.md](../app-server-observer/codex-plan-mirror.md) 繼續收斂 combined final reply 與 plan snapshot 的 transcript / UI 呈現細節。
3. 補寫並驗證 [app-server-ws-mirror-observer.md](../app-server-observer/app-server-ws-mirror-observer.md) 所描述的 observer contract，收斂剩餘 terminology 與 subscription 邊界。
