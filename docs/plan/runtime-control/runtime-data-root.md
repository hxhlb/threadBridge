...
這份文件定義 bot-local runtime state 與 installed runtime support 的本地路徑契約。

它處理：

- 預設 data root 的 build-profile 規則
- installed runtime support root 的 build-profile / bundle 規則
...
- `debug` build 的 source assets 預設使用 repo-local `./runtime_support`
- bundled `release` build 預設使用平台 local app-data dir 下的 `threadBridge/data`
- bundled `release` build 的 installed runtime support 預設使用平台 local app-data dir 下的 `threadBridge/runtime_support`
...
- macOS:
  ...
  - installed runtime support root: `~/Library/Application Support/threadBridge/runtime_support`
...
- 不自動搬移既有 repo-local `runtime_support/`
- bundled release 只在 installed runtime support 缺檔時，從 app bundle seed runtime support 補建
- 不建立 symlink
...
- `data/` 與 `runtime_support/` 都要維持 repo/source 與 bundled/install layout 同構
...
- bundled release 首次啟動可在 repo 外自動補齊缺失的 `runtime_support/`
