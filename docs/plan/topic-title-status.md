...
- 舊的 rename service message 不會 retroactive 回頭清理
...
## Rename Service Message Cleanup

Telegram forum topic 在改名後，會額外生成一條 service message，例如：

- `threadBridgeBot changed the topic name to "AGENTS.md Ready Check"`

如果 threadBridge 自己頻繁更新 title，這類訊息很容易把整個 thread 畫面洗滿。

目前已落地的 v1 語義：

- 只要 bot 收到的是 `forum_topic_edited` service message
- 而且它屬於 threadBridge 已管理的 Telegram thread
- bot 就會 best-effort 直接刪掉那條 service message

這個語義目前不區分：

- 是 bot 自己改名觸發
- 還是人在同一個 managed thread 裡手動改名觸發

目前刻意保持簡單，不做的事：

- 不保存這類 service message 的歷史 `message_id`
- 不補做刪除失敗後的下次重試
- 不回頭掃描已存在的舊 rename service message

所以這個能力的實際效果應理解成：

- 功能上線後新產生的 rename service message 會被立即清掉
- 但上線前或刪除失敗遺留的舊訊息，仍然會留在 Telegram 歷史裡
...
- Telegram 刪除權限不足時，rename service message cleanup 會安靜失效，只留 log
...
5. 如果之後仍覺得畫面噪音過高，再評估是否要引入 rename service message 的失敗重試或歷史追清機制。
